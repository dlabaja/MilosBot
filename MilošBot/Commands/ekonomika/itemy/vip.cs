...
{
    public class KoupitVIP : ModuleBase<SocketCommandContext>
    {
        private const int cena = 50_000;

        [Command("koupit-VIP")]
        [FormatSummary("Koupí V.I.P. za {0} peněz.", cena)]
        public async Task KoupitVIPAsync()
        {
            var najs = Context.User.Id.ToString();

            var role2 = Context.Guild.Roles.FirstOrDefault(x => x.Name == "V.I.P.");
            var ruka = Context.User as SocketGuildUser;

            if (ruka.Roles.Contains(role2))
            {
                await Context.Message.Channel.SendMessageAsync("Už nejspíše vlastníš VIP");
                return;
            }
            ...
            int uzivatel55 = cisla.item1;
            int penizeBanka = cisla.banka;
            if (uzivatel55 == 1)
            {
                await Context.Channel.SendMessageAsync("už nejspíše vlastníš V.I.P., ale někdo ti odebral roli, pro její navrácení napiš do #feedback děkuji");
                return;
            }
            if (penizeBanka < cena)
            {
                await Context.Channel.SendMessageAsync("nemáš dost peněz ke koupi VIP, až našetříš 50 000 můžeš to zkusit znovu");
                return;
            }
            await Context.Channel.SendMessageAsync("právě si jsi zakoupil VIP, gratuluji");
            ...
            var update1 = Builders<BsonDocument>.Update.Set("item1", slovo6);

            coll1.UpdateOne(filter, update);
            coll1.UpdateOne(filter, update1);

            var ovcak = new EmbedBuilder();
            ovcak.WithTitle($"V bance máš momentálně " + uzivatel2 + agr + " STORKSCOINS+vlastníš právě zakoupené VIP");
            ...
            await cons.SendMessageAsync("koupit VIP " + ruka);
        }
    }
}
