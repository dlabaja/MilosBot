...
{
    public class helposdeshdjklkt : ModuleBase<SocketCommandContext>
    {
        [Command("Prodat-BiTStorK")]
        public async Task Kickáásdsdsdsaswewdsaenzou([Remainder] string reason = null)
        {
            ...
            if (i >= 1)
            {
                ...
                DateTime hod1 = cisla12.timer5;
                int NSDM = cisla12.kryptomena2;
                ...
                if (porovnani >= hod1)
                {
                    ...
                    var coll191 = db.GetCollection<Rootobject1>("test1");
                    ...
                    await cons1.SendMessageAsync("Prodat BiTStorK " + ruka);
                    if (NSDM == 0)
                    {
                        await Context.Channel.SendMessageAsync("nevlastníš žádnou kryptoměnu BiTStorK");
                        return;
                    }
                    if (NSDM <= i)
                    {
                        ...
                        var update91 = Builders<Rootobject1>.Update.Set("kryptomena2", NSDM2);
                        ...
                        await Context.Channel.SendMessageAsync("prodal jsi všechnu svou kryptoměnu BiTStorK v kurzu 1:" + kurz + " za " + banka21);
                    }
                    if (NSDM > i)
                    {
                        var coll12 = db.GetCollection<Rootobject1>("kryptomena1");
                        var filter9 = Builders<Rootobject1>.Filter.Eq("uzivatelid", najs);
                        var update91 = Builders<Rootobject1>.Update.Set("kryptomena2", NSDM1);
                        coll12.UpdateOne(filter9, update91);
                        ...
                        coll121.UpdateOne(filter91, update941);
                        int banka12 = i * kurz;
                        await Context.Channel.SendMessageAsync("prodal jsi " + i + " své kryptoměny BiTStorK v kurzu 1:" + kurz + " za " + banka12);
                    }
                }
