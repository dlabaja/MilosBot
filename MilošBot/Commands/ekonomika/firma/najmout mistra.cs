...
{
    public class NajmoutMistra : ModuleBase<SocketCommandContext>
    {
        private const int cenaMistr = 15_000;

        [Command("najmout-mistra")]
        [Alias("mistr", "najmout-mis")]
        [FormatSummary("Najme mistra za {0} peněz.", cenaMistr)]
        public async Task NajmoutMistraAsync([Summary("Počet, nepovinný parametr."), Remainder] string count = null)
        {
            ...
            if (ruka.Roles.Contains(role2) || ruka.Roles.Contains(role3) || ruka.Roles.Contains(role4) || ruka.Roles.Contains(role5) || ruka.Roles.Contains(role6) || ruka.Roles.Contains(role7) || ruka.Roles.Contains(role8) || ruka.Roles.Contains(role9) || ruka.Roles.Contains(role10))
            {
                ...
                int uzivatel55 = cisla.mistri;
                int uzivatel57 = cisla.experti;
                ...
                if (uzivatel55 >= 0 && ruka.Roles.Contains(role3) || uzivatelx > 0 && ruka.Roles.Contains(role3))
                {
                ...
                if (uzivatel55 >= 1 && ruka.Roles.Contains(role4) || uzivatelx > 1 && ruka.Roles.Contains(role4))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 1. Pro postup na úroveň [strednifirma] musíš mít 21 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 5 && ruka.Roles.Contains(role5) || uzivatelx > 5 && ruka.Roles.Contains(role5))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 5. Pro postup na úroveň [vetsifirma] musíš mít 32 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 20 && ruka.Roles.Contains(role6) || uzivatelx > 20 && ruka.Roles.Contains(role6))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 20. Pro postup na úroveň [velkafirma] musíš mít 106 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 50 && ruka.Roles.Contains(role7) || uzivatelx > 50 && ruka.Roles.Contains(role7))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 50. Pro postup na úroveň [akciovaspolecnost] musíš mít 206 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 100 && ruka.Roles.Contains(role8) || uzivatelx > 100 && ruka.Roles.Contains(role8))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 100. Pro postup na úroveň [korporace] musíš mít 652 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 200 && ruka.Roles.Contains(role9) || uzivatelx > 200 && ruka.Roles.Contains(role9))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 200. Pro postup na úroveň [nadnarodnispolecnost] musíš mít 1584 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 350 && ruka.Roles.Contains(role10) || uzivatelx > 350 && ruka.Roles.Contains(role10))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 350. ");
                    return;
                }
                int uzivatel2 = 0;
                int slovo6 = 0;
                int mezisoucet = i * cenaMistr;
                if (uzivatel >= cenaMistr && mezisoucet == 0)
                {
                    await Context.Channel.SendMessageAsync("právě jsi si najmul mistra, gratuluji");
                    goto label11;
                }
                ...
                if (uzivatel <= (cenaMistr - 1) && i == 0 || i == 1)
                {
                ...
                label11:
                uzivatel2 = uzivatel - cenaMistr;  //doplnit do experta
                slovo6 = uzivatel55 + 1;
                goto label13;
                label12:
                ...
                var update1 = Builders<BsonDocument>.Update.Set("mistri", slovo6);
                ...
                await cons.SendMessageAsync("najmout mistra " + i + " " + ruka);
            }
            else
            {
                await Context.Channel.SendMessageAsync("mistra můžeš najímat až od strednemaléfirmy");
            }
        }
    }
}
...
namespace MilošBot.Commands.help
{
    public class helpoasdsadsadasdasdaaast : ModuleBase<SocketCommandContext>
    {
        [Command("najmout-mistra")]
        [Alias("mistr", "najmout-mis")]
        public async Task Mildosfdsadadseaggurus1()  //pridat kontrolu zivnosti
        {
            ...
            if (ruka.Roles.Contains(role4) || ruka.Roles.Contains(role5) || ruka.Roles.Contains(role6) || ruka.Roles.Contains(role7) || ruka.Roles.Contains(role8) || ruka.Roles.Contains(role9) || ruka.Roles.Contains(role10))
            {
                ...
                using (SqlConnection pripojeni = new SqlConnection(connectionString))
                {
                    ...
                    int uzivatel55 = (int)dotaz12.ExecuteScalar();
                    ...
                    if (uzivatel55 >= 1 && ruka.Roles.Contains(role4))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 1. Pro postup na úroveň [strednifirma] musíš mít 21 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 5 && ruka.Roles.Contains(role5))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 5. Pro postup na úroveň [vetsifirma] musíš mít 32 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 20 && ruka.Roles.Contains(role6))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 20. Pro postup na úroveň [velkafirma] musíš mít 106 jakýchkoliv pracovníků");
                        return;
                    }
                    ...
                    if (uzivatel55 >= 100 && ruka.Roles.Contains(role8))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 100. Pro postup na úroveň [korporace] musíš mít 652 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 200 && ruka.Roles.Contains(role9))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 200. Pro postup na úroveň [nadnarodnispolecnost] musíš mít 1584 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 350 && ruka.Roles.Contains(role10))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet mistrů které na této úrovni firmy můžeš zaměstnat je 350. ");
                        return;
                    }
                    ...
                    if (uzivatel <= 14999)
                    {
                        await Context.Channel.SendMessageAsync("nemáš dost peněz v bance na zaplacení mistra, až našetříš 15 000 můžeš to zkusit znovu");
                        return;
                    }

                label11:
                    int uzivatel2 = uzivatel - 15000;
                    int slovo6 = uzivatel55 + 1;

                    SqlCommand dotaz11 = new SqlCommand();
                    dotaz11.Connection = pripojeni;
                    dotaz11.CommandText = "UPDATE Kam SET banka=@slovo5, mistri=@slovo6 WHERE uzivatelid=@slovo1";
                    ...
                    //log
                    ITextChannel cons = Context.Client.GetChannel(814954423972266024) as ITextChannel;
                    await cons.SendMessageAsync("najmout mistra " + ruka);
                }
            }
            else
            {
