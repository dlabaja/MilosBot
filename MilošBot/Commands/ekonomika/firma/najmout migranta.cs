...
{
    public class NajmoutMigranta : ModuleBase<SocketCommandContext>
    {
        private const int cenaMig = 5_000;

        [Command("najmout-migranta")]
        [Alias("migrant", "najmout-mig")]
        [FormatSummary("Najme migranta za {0} peněz.", cenaMig)]
        public async Task NajmoutMigrantaAsync([Summary("Počet, nepovinný parametr."), Remainder] string count = null)
        {
            ...
            if (ruka.Roles.Contains(role2) || ruka.Roles.Contains(role3) || ruka.Roles.Contains(role4) || ruka.Roles.Contains(role5) || ruka.Roles.Contains(role6) || ruka.Roles.Contains(role7) || ruka.Roles.Contains(role8) || ruka.Roles.Contains(role9) || ruka.Roles.Contains(role10))
            {
                ...
                int uzivatel55 = cisla.pracujicimigranti;
                ...
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 5. Pro postup na úroveň [středně malá firma] musíš mít 8 jakýchkoliv pracovníků");
                    return;
                }
                ...
                if (uzivatel55 >= 10 && ruka.Roles.Contains(role4) || uzivatelx > 10 && ruka.Roles.Contains(role4))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 10. Pro postup na úroveň [strednifirma] musíš mít 21 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 10 && ruka.Roles.Contains(role5) || uzivatelx > 10 && ruka.Roles.Contains(role5))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 10. Pro postup na úroveň [vetsifirma] musíš mít 32 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 10 && ruka.Roles.Contains(role6) || uzivatelx > 10 && ruka.Roles.Contains(role6))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 10. Pro postup na úroveň [velkafirma] musíš mít 106 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 50 && ruka.Roles.Contains(role7) || uzivatelx > 50 && ruka.Roles.Contains(role7))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 50. Pro postup na úroveň [akciovaspolecnost] musíš mít 206 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 200 && ruka.Roles.Contains(role8) || uzivatelx > 200 && ruka.Roles.Contains(role8))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 200. Pro postup na úroveň [korporace] musíš mít 652 jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 500 && ruka.Roles.Contains(role9) || uzivatelx > 500 && ruka.Roles.Contains(role9))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 500. Pro postup na úroveň [nadnarodnispolecnost] musíš mít 1584  jakýchkoliv pracovníků");
                    return;
                }
                if (uzivatel55 >= 1000 && ruka.Roles.Contains(role10) || uzivatelx > 1000 && ruka.Roles.Contains(role10))
                {
                    await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 1000. ");
                    return;
                }
                //od
                int uzivatel2 = 0;
                int slovo6 = 0;
                int mezisoucet = i * cenaMig;
                if (uzivatel >= cenaMig && mezisoucet == 0)
                {
                    await Context.Channel.SendMessageAsync("právě jsi si najmul migranta, gratuluji");
                    goto label11;
                }
                ...
                if (uzivatel <= (cenaMig - 1) && i == 0 || i == 1)
                {
                    await Context.Channel.SendMessageAsync("nemáš dost peněz na černé ruce na zaplacení migranta, až našetříš 5 000 můžeš to zkusit znovu");
                    return;
                }
                if (uzivatel < mezisoucet)
                {
                    await Context.Channel.SendMessageAsync("nemáš dost peněz na černé ruce na zaplacení " + i + " migrantů, až našetříš " + mezisoucet + " můžeš to zkusit znovu");
                    return;
                }

                label11:
                uzivatel2 = uzivatel - cenaMig;  //doplnit do experta
                slovo6 = uzivatel55 + 1;
                goto label13;
                label12:
                ...
                var update1 = Builders<BsonDocument>.Update.Set("pracujicimigranti", slovo6);
                ...
                await cons.SendMessageAsync("najmout migranta " + i + " " + ruka);
            }
            else
            {
                await Context.Channel.SendMessageAsync("Migranty můžeš najímat až od vlastnictví živnosti");
            }
        }
    }
}/*
...
namespace MilošBot.Commands.help
{
    public class helpoasdaaast : ModuleBase<SocketCommandContext>
    {
        [Command("najmout-migranta")]
        [Alias("migrant", "najmout-mig")]
        public async Task Mildosfdsadadseaggurus1()
        {
            ...
            if (ruka.Roles.Contains(role3) || ruka.Roles.Contains(role2) || ruka.Roles.Contains(role4) || ruka.Roles.Contains(role5) || ruka.Roles.Contains(role6) || ruka.Roles.Contains(role7) || ruka.Roles.Contains(role8) || ruka.Roles.Contains(role9) || ruka.Roles.Contains(role10))
            {
                ...
                using (SqlConnection pripojeni = new SqlConnection(connectionString))
                {
                    ...
                    dotaz.CommandText = @"SELECT pracujicimigranti FROM dbo.Kam WHERE uzivatelid=@slovo1";
                    ...
                    dotaz1.CommandText = @"SELECT zamestnanci FROM dbo.Kam WHERE uzivatelid=@slovo1";
                    ...
                    if (uzivatel55 >= 10 && ruka.Roles.Contains(role4))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 10. Pro postup na úroveň [strednifirma] musíš mít 21 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 10 && ruka.Roles.Contains(role5))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 10. Pro postup na úroveň [vetsifirma] musíš mít 32 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 10 && ruka.Roles.Contains(role6))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 10. Pro postup na úroveň [velkafirma] musíš mít 106 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 50 && ruka.Roles.Contains(role7))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 50. Pro postup na úroveň [akciovaspolecnost] musíš mít 206 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 200 && ruka.Roles.Contains(role8))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 200. Pro postup na úroveň [korporace] musíš mít 652 jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 500 && ruka.Roles.Contains(role9))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 500. Pro postup na úroveň [nadnarodnispolecnost] musíš mít 1584  jakýchkoliv pracovníků");
                        return;
                    }
                    if (uzivatel55 >= 1000 && ruka.Roles.Contains(role10))
                    {
                        await Context.Channel.SendMessageAsync("Maximální počet migrantů které na této úrovni firmy můžeš zaměstnat je 1000. ");
                        return;
                    }
                    ...
                    if (uzivatel <= 4999)
                    {
                        await Context.Channel.SendMessageAsync("nemáš dost peněz na černéruce na zaplacení prvního migranta, až našetříš 5 000 můžeš to zkusit znovu");
                        return;
                    }

                label11:
                    int uzivatel2 = uzivatel - 5000;
                    int slovo6 = uzivatel55 + 1;

                    SqlCommand dotaz11 = new SqlCommand();
                    dotaz11.Connection = pripojeni;
                    dotaz11.CommandText = "UPDATE Kam SET cernaruka=@slovo5, pracujicimigranti=@slovo6 WHERE uzivatelid=@slovo1";
                    ...
                    //log
                    ITextChannel cons = Context.Client.GetChannel(814954423972266024) as ITextChannel;
                    await cons.SendMessageAsync("najmout migranta " + ruka);
                }
            }
            else
            {
